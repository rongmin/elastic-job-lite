...
function renderJobsOverview() {
    ...
    if (activated) {
        jsonData.url = "/api/jobs/specifiedJobs";
    }
    ...
    $("#jobs-status-overview-tbl2").bootstrapTable(myoptions).on("all.bs.table", function() {
        doLocale();
    });
}

function refreshOptions() {
	$("#jobs-status-overview-tbl2").bootstrapTable('refreshOptions', myoptions)
}
...
function bindTriggerButton() {
    ...
    $(document).on("click", "button[operation='trigger-job'][data-toggle!='modal']", function(event) {
        ...
        $.ajax({
            ...
            success: function() {
                showSuccessDialog();
                $("#jobs-status-overview-tbl2").bootstrapTable("refresh");
            }
        });
    });
}
